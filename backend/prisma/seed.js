  ...
  await prisma.$transaction(async (tx) => {
    ...
    const players = await Promise.all(
      [
        ...
        ["Bobby", "Fischer", 1400],
        ["Hou", "Yifan", 1500],
        ["Viswanathan", "Anand", 1600],
        ["Vladimir", "Kramnik", 1700],
      ].map(async ([firstName, lastName, rating], i) =>
        tx.user.upsert({
    ...
    async function createTournament({
      ...
      // --- Round 1 ---
      const round1 = await tx.round.create({
        data: {
          tournamentId: tournament.id,
          roundNumber: 1,
          title: "Quarterfinals",
      ...
      // --- Round 2 ---
      const round2 = await tx.round.create({
        data: {
          tournamentId: tournament.id,
          roundNumber: 2,
          title: "Semifinals",
      ...
      // --- Round 3 ---
      const round3 = await tx.round.create({
        data: {
          tournamentId: tournament.id,
          roundNumber: 3,
          title: "Final",
